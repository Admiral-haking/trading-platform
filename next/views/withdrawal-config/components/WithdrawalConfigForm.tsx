...
export type WithdrawalConfigValues = z.infer<typeof schema>;

type Props = {
  defaults?: Partial<WithdrawalConfigValues>;
  onDone: () => void;
};
...
export default function WithdrawalConfigForm({ defaults, onDone }: Props) {
  ...
  const onSubmit = methods.handleSubmit(async (values) => {
    ...
    setSaving(true);
    try {
      // Save as strings to match backend config style
  ...
  return (
    <FormProvider {...methods}>
      <Box component="form" onSubmit={onSubmit} noValidate>
        <Stack spacing={2}>
          <Typography variant="h6" sx={{ fontWeight: 700 }}>Withdrawal Configuration</Typography>
          <RHFSelect name="withdrawalDayOfWeek" label="Withdrawal Day (0-6)" fullWidth>
            {days.map((d) => (
