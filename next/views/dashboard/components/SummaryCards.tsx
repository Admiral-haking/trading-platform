...
function Stat({ label, value, color = 'success.main' as any }: { label: string; value: string | number; color?: any }) {
  return (
    <Card sx={{ m: 1, height: '100%', background: (t) => t.palette.mode === 'dark' ? 'linear-gradient(180deg, rgba(0,229,168,0.08), rgba(124,77,255,0.05))' : 'linear-gradient(180deg, rgba(12,123,232,0.06), rgba(123,70,255,0.04))' }}>
      <CardContent>
...
export default function SummaryCards({ spotAvailableUSDT, spotFrozenUSDT, featuresAvailableUSDT, featuresFrozenUSDT }: Props) {
  return (
    <Grid container sx={{ maxWidth: '95vw' }}>
